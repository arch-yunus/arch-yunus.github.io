<div align="center">

# ⚡ BAHATTİN YUNUS ÇETİN
### **AI Developer · UAV Systems Architect · Embedded Systems Engineer**
**Founder @ Arat Labs · Tulpar Aero-Dynamics · Arkıl Games · MERL · Budak · Anka Silicon Dynamics**

[![Live Demo](https://img.shields.io/badge/LIVE_SYSTEM-arch--yunus.github.io-00f3ff?style=for-the-badge&logo=google-chrome&logoColor=black)](https://arch-yunus.github.io/)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-12.6K+-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/bahattinyunus/)
[![GitHub Repos](https://img.shields.io/badge/GitHub_Repos-231+-24292e?style=for-the-badge&logo=github&logoColor=white)](https://github.com/arch-yunus)
[![Medium](https://img.shields.io/badge/Medium-Articles-black?style=for-the-badge&logo=medium&logoColor=white)](https://medium.com/@archyunus)

<br/>

```text
  ██████╗ ███████╗████████╗██████╗  ██████╗ ███████╗ ██████╗ ██╗      ██╗███████╗
 ██╔════╝ ██╔════╝╚══██╔══╝██╔══██╗██╔═══██╗██╔════╝██╔═══██╗██║      ██║██╔════╝
 ██║  ███╗█████╗     ██║   ██████╔╝██║   ██║███████╗██║   ██║██║      ██║███████╗
 ██║   ██║██╔══╝     ██║   ██╔══██╗██║   ██║╚════██║██║   ██║██║      ██║╚════██║
 ╚██████╔╝███████╗   ██║   ██████╔╝╚██████╔╝███████║╚██████╔╝███████╗██║███████║
  ╚═════╝ ╚══════╝   ╚═╝   ╚═════╝  ╚═════╝ ╚══════╝ ╚═════╝ ╚══════╝╚═╝╚══════╝
                            METAL YAKA // PROTOCOL v5.0
```

</div>

---

## 🌌 Genel Bakış (Overview)

Bu repo; **Otonom İHA/SİHA Sistemleri**, **Büyük Dil Modelleri (LLM/Knowledge Distillation)**, **Edge-AI Optimizasyonu (%80 Pruning)**, **Gömülü Sistemler (RTOS/RISC-V)** ve **Siber Savunma** disiplinlerinde geliştirilen yüksek teknolojili Ar-Ge projelerini, girişimleri, interaktif telemetri simülatörlerini ve akademik faaliyetleri sergileyen yeni nesil **Cyberpunk / Metal Yaka v5.0** portföy sistemidir.

🔗 **Canlı Sistem:** [https://arch-yunus.github.io/](https://arch-yunus.github.io/)

---

## 🛸 Temel İnteraktif Modüller & Özellikler

### 1. 🛫 Tulpar UAV Flight Deck (`TulparFlightDeck.tsx`)
- **Fiziksel Uçuş Telemetrisi:** Gerçek zamanlı yapay ufuk (artificial horizon), pitch/roll dinamikleri, dijital airspeed ve barometrik irtifa göstergeleri.
- **Saha Protokolleri:** ROS2 Foxy/Humble, MAVLink2 çift yönlü telemetri köprüsü ve PX4 SITL otonom waypoint HUD simülasyonu.

### 2. ⚡ Budak Edge-AI Pruning Lab (`BudakEdgeLab.tsx`)
- **%80 Model Budama (Pruning):** Yapılandırılmış ve yapılandırılmamış tensör budama motoru.
- **Donanım Seviyesinde Gizlilik:** FP16, INT8 ve INT4 nicemleme (Quantization) ile canlı VRAM tasarrufu ve gecikme (latency) hesaplayıcısı. Sıfır bulut sızıntısı.

### 3. 📜 120+ Sertifika & Akreditasyon Kasası (`CertificationVault.tsx`)
- **Doğrulanmış Kayıtlar:** SHGM İHA-1 Pilotaj Lisansı, TEI Havacılık Motorları Okulu, TÜBİTAK/GSB KAMP+ Uzay (Radyo Astronomi), BGD KAMP+ Siber Savunma, TUA Astro Hackathon, BTK Akademi ve Bilgeİş akreditasyonları için filtreli ve anlık aramalı dijital kasa.

### 4. 🧠 Vanguard AI Taktik Asistanı (`VanguardAI.tsx`)
- **Siber Asistan:** Yunus Çetin'in tüm teknik yetenekleri, girişimleri, açık kaynak repoları, makaleleri ve akademik geçmişiyle beslenen yerel bilgi tabanı.

---

## 🏢 Kurucu Girişimler & Ar-Ge Ekosistemi

| Girişim | Rol | Odak Alanı & Açıklama |
| :--- | :--- | :--- |
| **Arat Labs** | Kurucu & Baş Yazılım Mühendisi | Next-Gen AI Architectures, asimetrik sistem mühendisliği ve derin teknoloji Ar-Ge'si. |
| **Tulpar Aero-Dynamics** | Kurucu & Baş Teknoloji Mimarı | UE5 fizik tabanlı uçuş simülasyonları, İHA/SİHA ve eVTOL otonom uçuş algoritmaları. |
| **Arkıl Games** | Kurucu & Yaratıcı Direktör | Türk mitolojisi tabanlı evren tasarımı ve prosedürel oyun motoru mimarisi. |
| **MERL (Meta-Engineering Research Lab)** | Baş Araştırmacı & Kurucu | İnsan müdahalesiz otonom kod fabrikaları & stokastik zeka modelleri. |
| **Budak \| Edge-AI Optimization** | Kurucu & Baş Araştırma Mühendisi | %80 Model budama & INT8/INT4 donanım seviyesinde veri gizliliği kalkanı. |
| **Anka Silicon Dynamics** | Kurucu & Baş Sistem Mimarı | Knowledge Distillation ile donanım bağımsız zeka & milli NPU/FPGA simülasyonları. |

---

## 🎓 Akademik Matris & Akreditasyonlar

- 🏫 **Orta Doğu Teknik Üniversitesi (ODTÜ)** — Fizik Öğretmenliği (İngilizce Hazırlık, 2026 – Devam Ediyor)
- 🏫 **Hacettepe Üniversitesi** — Alternatif Enerji Kaynakları Teknolojisi (2026 – Devam Ediyor)
- 🏫 **Karadeniz Teknik Üniversitesi** — Yazılım Mühendisliği (2023 – 2026)
- 🏫 **İstanbul Üniversitesi** — Yönetim Bilişim Sistemleri (AUZEF, 2025 – 2029)
- 🏫 **Anadolu Üniversitesi** — Bilgisayar Programcılığı (AÖF, 2025 – 2027)
- 🏫 **Atatürk Üniversitesi** — Tarih (ATA-AÖF, 2026 – 2030)
- 🎖️ **SHGM:** İHA-1 Sportif/Amatör Pilotaj Lisansı
- 🎖️ **TEI:** 6. Dönem Havacılık Motorları Okulu (Gaz Türbinli Motorlar)
- 🎖️ **TÜBİTAK & GSB:** KAMP+ Havacılık ve Uzay Kampı (Bursa - FezaArsivi)
- 🎖️ **BGD & GSB:** KAMP+ Siber Güvenlik Kampı (SOC, Pentest & Adli Bilişim)
- 🎖️ **Türkiye Uzay Ajansı (TUA):** Astro Hackathon (15 CubeSat / Derin Uzay Görevi)

---

## 💻 Amiral Gemisi Açık Kaynak Repolar

- **[BeeRoute](https://github.com/arch-yunus/BeeRoute):** Bio-mimetic Yapay Arı Kolonisi (ABC) NP-Hard rota optimizasyonu ($O(G \cdot S \cdot n^2)$).
- **[Tulpar-Aero-Simulator](https://github.com/arch-yunus/tulpar-aero-dynamics):** UE5 & ROS2 telemetri ve aerodinamik uçuş simülatörü.
- **[Budak-Edge-Pruner](https://github.com/arch-yunus/budak-edge-ai):** PyTorch/ONNX %80 model budama ve INT8/INT4 cihaz üstü gizlilik motoru.
- **[Anka-Distill-Core](https://github.com/arch-yunus/anka-silicon-dynamics):** 70B teacher modellerini 7B/8B student modellerine aktaran Knowledge Distillation mimarisi.
- **[FezaArsivi](https://github.com/arch-yunus/FezaArsivi):** TÜBİTAK/TUA destekli radyo astronomi ve CubeSat telemetri merkezi.
- **[Aura-Hockey-AR](https://github.com/arch-yunus/Aura-Hockey-AR):** MediaPipe & Three.js 3D WebGL Ankara AI Hackathon AR projesi.
- **[Celik-MCU-RISCV-SDK](https://github.com/arch-yunus/Celik-MCU-SDK):** İlk yerli RISC-V çipi Çelik MCU için gömülü C++ SDK ve RTOS HAL katmanı.
- **[Trabzon-Protocol-MQTT](https://github.com/arch-yunus/Trabzon-Protocol):** Denizcilik ve İHA telemetrisi için hafif asenkron ikili haberleşme protokolü.
- **[Arkil-Mythos-Engine](https://github.com/arch-yunus/arkil-games):** Türk mitolojisi tabanlı prosedürel evren ve UE5 C++ davranış ağaçları motoru.
- **[Math2Code-Guide](https://github.com/arch-yunus/Math2Code-A-Practical-Guide-for-Developers):** Matematiksel modelleri yüksek başarımlı koda dönüştürme rehberi (200+ Yıldız).

---

## 🛠️ Teknoloji Yığını (Tech Stack)

| Kategori | Teknolojiler |
| :--- | :--- |
| **Frontend & UI** | React 19, TypeScript, Tailwind CSS 4, Shadcn/UI, Lucide React, Framer Motion |
| **Grafik & 3D** | Three.js, WebGL, Canvas API, Unreal Engine 5 |
| **Yapay Zeka & ML** | PyTorch, TensorFlow, Unsloth, vLLM, QLoRA, TFLite, ONNX Runtime |
| **Robotik & Havacılık** | ROS2 (Humble/Foxy), PX4 SITL, Gazebo, MAVLink2, Microstrip RF |
| **Gömülü & Diller** | C++17/20, C, Python, Rust, Go, RISC-V Assembly, FreeRTOS |
| **Siber Güvenlik** | Zero Trust, Pwntools, Ghidra, Scapy, AES-256-GCM, Linux Cgroups |

---

## 🏃‍♂️ Yerel Kurulum & Çalıştırma

```bash
# 1. Depoyu klonlayın
git clone https://github.com/arch-yunus/arch-yunus.github.io.git
cd arch-yunus.github.io

# 2. Bağımlılıkları yükleyin
npm install

# 3. Geliştirme sunucusunu başlatın
npm run dev

# 4. Üretim sürümü için derleyin
npm run build
```

---

## 📬 İletişim & Ağ

- **E-Posta:** [bahattinyunuscetin@hotmail.com](mailto:bahattinyunuscetin@hotmail.com)
- **LinkedIn:** [linkedin.com/in/bahattinyunus](https://www.linkedin.com/in/bahattinyunus/) (12.644+ Bağlantı & Takipçi)
- **GitHub:** [github.com/arch-yunus](https://github.com/arch-yunus) (231+ Kamu Reposu)
- **Medium:** [medium.com/@archyunus](https://medium.com/@archyunus)

---

<div align="center">
  <sub>Geliştirici: <strong>Bahattin Yunus ÇETİN</strong> · Ankara, Türkiye · MIT Lisansı</sub>
</div>
